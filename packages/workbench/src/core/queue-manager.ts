...
/** BullMQ keeps the redis client on a non-public field; expose it by name. */
type QueueWithClient = Queue & { client?: Promise<SortedSetClient> };

/**
 * The job states we fetch and render. Every member is both a BullMQ `JobType`
 * (valid for `queue.getJobs`) and a `JobStatus` (valid for our UI), so it
 * crosses that boundary without a cast.
 */
const FETCHABLE_JOB_TYPES = [
  'active',
  'waiting',
  'waiting-children',
  'prioritized',
...
type FetchableJobType = (typeof FETCHABLE_JOB_TYPES)[number];

/** Default set fetched when no status filter is applied (excludes `paused`). */
const DEFAULT_FETCH_TYPES = [
  'active',
  'waiting',
  'waiting-children',
  'prioritized',
  ...
  'failed',
  'delayed',
] as const satisfies readonly FetchableJobType[];

const FETCHABLE_JOB_TYPE_SET: ReadonlySet<string> = new Set(
  FETCHABLE_JOB_TYPES,
);

function isFetchableJobType(status: JobStatus): status is FetchableJobType {
  return FETCHABLE_JOB_TYPE_SET.has(status);
}

/** A job paired with the state it was fetched under. */
type JobWithState = { job: Job; state: JobStatus };

/**
 * Job options as BullMQ stores them on scheduled/repeatable jobs — `JobsOptions`
 * plus the runtime-only bookkeeping keys it adds, which we strip before re-adding.
 */
type StoredJobOpts = JobsOptions & {
  repeatJobKey?: unknown;
  prevMillis?: unknown;
  timestamp?: unknown;
};

function jobRegistryId(job: { queue: string; name: string }): string {
  return `${job.queue}/${job.name}`;
}

/**
 * Compute the next `count` run timestamps for a scheduler. Cron schedules are
 * expanded with `cron-parser` (honoring the scheduler's timezone); interval
 * schedules step forward by `every` from the next known run. Returns whatever
 * could be computed — an unparseable pattern yields an empty list.
 */
  ...
  if (scheduler.pattern) {
    try {
      ...
      for (let i = 0; i < count; i++) {
        runs.push(iterator.next().toDate().getTime());
      }
    } catch {
      // Unparseable pattern — fall through to whatever we have (nothing).
    }
    return runs;
  }

  if (scheduler.every) {
    const start = scheduler.next ?? Date.now() + scheduler.every;
    for (let i = 0; i < count; i++) {
      runs.push(start + i * scheduler.every);
    }
  }
...
function getTagValue(
  ...
  const scopes = [
    root,
    objectRecord(root.__input),
    objectRecord(root.__meta),
    objectRecord(root.__metadata),
  ];
...
function withRootOpts(flow: unknown, opts: TestJobRequest['opts']): unknown {
  if (!opts || !isRecord(flow)) return flow;
  const rootOpts = isRecord(flow.opts) ? flow.opts : {};
  return {
    ...flow,
    opts: {
      ...rootOpts,
...
export class QueueManager {
  ...
  private registry?: WorkbenchRegistry;
  ...
  private async cached<T>(
    ...
    // Heterogeneous memo: each key stores a different `T`, so the cache itself
    // can only be typed as `unknown` — the caller's `T` is the source of truth.
  ...
  private getErrorClass(job: Job): string {
    const stackHead = job.stacktrace?.find(Boolean);
    const stackMatch = stackHead?.match(
  ...
  private async getJobsByTimeRange(
    ...
    try {
      const client = await (queue as QueueWithClient).client;
      if (!client) {
        // Fallback to regular getJobs if client not available
        const jobs = await queue.getJobs([status], offset, offset + limit - 1);
        return jobs.filter(
          (job): job is Job =>
            job != null &&
            !!job.finishedOn &&
      ...
      const queueKey = queue.toKey(status);
  ...
  getRegistryConfig(): WorkbenchRegistryConfig {
    if (!this.registry) {
      return { jobs: [], flows: [] };
    }
    ...
    const flows = (this.registry.flows ?? [])
      .map((flow) => ({
        type: 'flow' as const,
        id: flow.id,
        name: flow.name,
        ...
        (async () => {
          ...
          for (const { queueName, completedJobs, failedJobs } of queueResults) {
            ...
            for (const job of completedJobs) {
              ...
              const bucket = metrics.buckets[bucketIndex];
              const durations = metrics.durations[bucketIndex];
              const waitTimes = metrics.waitTimes[bucketIndex];
              if (bucket && durations && waitTimes) {
                bucket.completed++;
              ...
              // Track job type totals for error-rate denominators.
              const key = `${queueName}:${job.name}`;
              const stats = jobTypeTotals.get(key) || {
                name: job.name,
                queueName,
                completed: 0,
                failed: 0,
            ...
            for (const job of failedJobs) {
              ...
              // Track job type totals and error-class groups.
              const totalKey = `${queueName}:${job.name}`;
              const totals = jobTypeTotals.get(totalKey) || {
                name: job.name,
                queueName,
                completed: 0,
                failed: 0,
              ...
              totals.failed++;
              jobTypeTotals.set(totalKey, totals);

              const errorClass = this.getErrorClass(job);
              const failureKey = `${queueName}:${job.name}:${errorClass}`;
              const stats = failureStats.get(failureKey) || {
                name: job.name,
                queueName,
                errorClass,
                failed: 0,
              ...
              stats.failed++;
              const failureBucket = stats.buckets[bucketIndex];
              if (failureBucket !== undefined) {
                stats.buckets[bucketIndex] = failureBucket + 1;
              }
              if (
                job.id &&
                ...
                stats.latestFailedReason = job.failedReason;
              }
              failureStats.set(failureKey, stats);
            }
          }
          ...
          for (const metrics of queueMetricsMap.values()) {
            for (let i = 0; i < 24; i++) {
              const aggregateBucket = aggregateBuckets[i];
              const bucket = metrics.buckets[i];
              const durations = metrics.durations[i];
              const waitTimes = metrics.waitTimes[i];
              const aggregateDuration = aggregateDurations[i];
              const aggregateWaitTime = aggregateWaitTimes[i];
              if (
                !aggregateBucket ||
                !bucket ||
                !durations ||
              ...
              aggregateBucket.completed += bucket.completed;
              aggregateBucket.failed += bucket.failed;
              aggregateDuration.push(...durations);
              aggregateWaitTime.push(...waitTimes);
            }
          }
          ...
          for (let i = 0; i < 24; i++) {
            const aggregateBucket = aggregateBuckets[i];
            const durations = aggregateDurations[i];
            const waitTimes = aggregateWaitTimes[i];
            if (!aggregateBucket || !durations || !waitTimes) continue;
            if (durations.length > 0) {
              aggregateBucket.avgDuration = Math.round(
                durations.reduce((a, b) => a + b, 0) / durations.length,
              );
            }
            if (waitTimes.length > 0) {
              aggregateBucket.avgWaitTime = Math.round(
                waitTimes.reduce((a, b) => a + b, 0) / waitTimes.length,
              );
            }
          }
          ...
          const mostFailingTypes = Array.from(failureStats.values())
            .filter((s) => s.failed > 0)
            .map((s) => {
              const totals = jobTypeTotals.get(`${s.queueName}:${s.name}`);
              const totalCount =
                (totals?.completed ?? 0) + (totals?.failed ?? s.failed);
              ...
              return {
                ...
                failCount: s.failed,
                totalCount,
                errorRate: totalCount > 0 ? s.failed / totalCount : 0,
                errorClass: s.errorClass,
                latestFailedReason: s.latestFailedReason,
                latestFailedAt: s.latestFailedAt,
                trend: s.buckets,
              };
            })
  ...
  async getJobs(
    ...
    const types: readonly FetchableJobType[] = status
      ? isFetchableJobType(status)
        ? [status]
        : []
      : DEFAULT_FETCH_TYPES;
    ...
    const mergedStatuses = types.length > 1;
    const fetchStart = mergedStatuses ? 0 : start;
    const fetchEnd = Math.max(
      fetchStart,
      (mergedStatuses ? start + limit : start + limit) - 1,
    );

    for (const type of types) {
      const typeJobs = await queue.getJobs(type, fetchStart, fetchEnd);
      jobsWithState.push(...typeJobs.map((job) => ({ job, state: type })));
    ...
    const data = mergedStatuses
      ? jobInfos.slice(start, start + limit)
      : jobInfos.slice(0, limit);
  ...
  private jobMatchesAllFilters(
    ...
    if (filters.tags && Object.keys(filters.tags).length > 0) {
      ...
      for (const [field, value] of Object.entries(filters.tags)) {
        const jobValue = getTagValue(job.data, field);
  ...
  async search(query: string, limit = 20): Promise<SearchResult[]> {
    ...
    const types = DEFAULT_FETCH_TYPES;
          ...
          types.map(async (type) => {
            try {
              ...
              for (const job of jobs) {
                ...
                matches.push({
                  queue: queueName,
                  job: await this.jobToInfo(job, 'full', type),
                });
              }
  ...
  private async getLatestRuns(
    ...
    const perTypeFetch = Math.max(5, start + limit + 1);

    // Fetch from all queues in parallel and keep the originating state.
    const allTypes = DEFAULT_FETCH_TYPES;

    const results = await Promise.all(
      queueEntries.map(async ([queueName, queue]) => {
        const jobArrays = await Promise.all(
          allTypes.map(async (type) => {
            const jobs = await queue.getJobs(type, 0, perTypeFetch - 1);
  ...
  async getAllRuns(
    ...
    const types: readonly FetchableJobType[] = filters?.status
      ? isFetchableJobType(filters.status)
        ? [filters.status]
        : []
      : DEFAULT_FETCH_TYPES;
    ...
    const allJobs: { job: Job; queueName: string; state: JobStatus }[] = [];
    const seenJobIds = new Set<string>();
    const targetCount = start + limit + 1;
    const batchSize = Math.max(Math.ceil((limit * 2) / numQueues) + 3, 25);
    ...
    const fetchFromQueue = async (
      ...
      jobs: { job: Job; queueName: string; state: JobStatus }[];
      exhausted: boolean;
    }> => {
      ...
      if (hasTimeRange && filters?.timeRange) {
        ...
        if (types.includes('completed')) {
          ...
          batchCounts.push(completedJobs.length);
          timeRangeJobs.push(
            ...completedJobs.map(
              (job): JobWithState => ({ job, state: 'completed' }),
            ),
          );
        }
        ...
        if (types.includes('failed')) {
          ...
          batchCounts.push(failedJobs.length);
          timeRangeJobs.push(
            ...failedJobs.map(
              (job): JobWithState => ({ job, state: 'failed' }),
            ),
          );
        }
        ...
        if (otherTypes.length > 0) {
          const otherJobArrays = await Promise.all(
            otherTypes.map(async (type) => {
              ...
              batchCounts.push(jobs.length);
              return jobs.map((job): JobWithState => ({ job, state: type }));
            }),
          );
        ...
        return {
          jobs: timeRangeJobs.map(({ job, state }) => ({
            job,
            queueName,
            state,
          })),
          exhausted: batchCounts.every((batchCount) => batchCount < count),
        };
      }
      ...
      if (filters?.status) {
        ...
        const jobs = isFetchableJobType(status)
          ? await queue.getJobs(status, offset, offset + count - 1)
          : [];
        return {
          jobs: jobs.map((job) => ({ job, queueName, state: status })),
          exhausted: jobs.length < count,
        };
      }
      ...
      const jobArrays = await Promise.all(
        types.map(async (type) => {
          const jobs = await queue.getJobs(type, offset, offset + count - 1);
          batchCounts.push(jobs.length);
          return jobs.map((job): JobWithState => ({ job, state: type }));
        }),
      );
    ...
    let exhausted = false;
    while (
      (isTimestampSort ? allJobs.length < targetCount : true) &&
      !exhausted
    ) {
      const results = await Promise.all(
        queueEntries.map(([queueName, queue]) =>
          fetchFromQueue(queueName, queue, offset, batchSize),
        ),
      );

      exhausted = results.every((result) => result.exhausted);
      const batch = results.flatMap((result) => result.jobs);
      if (batch.length === 0) break;

      for (const entry of batch) {
        if (!entry.job.id) continue;

        const key = `${entry.queueName}:${entry.job.id}`;
        if (seenJobIds.has(key)) continue;
        seenJobIds.add(key);

        if (!filters || this.jobMatchesAllFilters(entry.job, filters)) {
          allJobs.push(entry);
        }
      }
    ...
    const runInfos = await Promise.all(
      (isTimestampSort ? allJobs.slice(start, start + limit) : allJobs).map(
        async ({ job, queueName, state }) => {
          const info = await this.jobToInfo(job, 'list', state);
          return { ...info, queueName };
        },
      ),
    ...
    const page = isTimestampSort
      ? runInfos
      : runInfos.slice(start, start + limit);

    // Build cursor for next page
    const hasMore = isTimestampSort
      ? allJobs.length > start + limit
      : runInfos.length > start + limit;

    return {
      data: page,
      total: -1, // Don't calculate total - not needed for UI
      hasMore,
      cursor: hasMore ? String(start + page.length) : undefined,
    };
  }
  ...
  /**
   * Detailed view of a single repeatable scheduler: its config, the next few
   * scheduled runs, and the most recent executions it produced. Returns null
   * when the queue or scheduler key is unknown.
   */
  async getSchedulerDetail(
    queueName: string,
    key: string,
  ): Promise<SchedulerDetail | null> {
    const queue = this.queues.get(queueName);
    if (!queue) return null;
    ...
    const pattern = scheduler.pattern ?? undefined;
    const every = scheduler.every ?? undefined;

    return {
      key: scheduler.key,
      name: scheduler.name || 'unnamed',
      queueName,
      pattern,
      every,
      ...
      type: pattern ? 'cron' : 'interval',
      upcoming: upcomingRuns(scheduler, 5),
      recent: await this.getSchedulerRuns(queue, queueName, key, 5),
    };
  }

  /**
   * Recent executions produced by a scheduler. Jobs spawned from a scheduler
   * carry `repeatJobKey === scheduler.key`, so we scan the most recent
   * completed and failed jobs on the queue and keep the matching ones.
   */
  private async getSchedulerRuns(
    ...
    const matches = [
      ...completed.map((job): JobWithState => ({ job, state: 'completed' })),
      ...failed.map((job): JobWithState => ({ job, state: 'failed' })),
    ]
      .filter(({ job }) => job.repeatJobKey === schedulerKey)
      .sort(
        (a, b) =>
          (b.job.finishedOn ?? b.job.timestamp) -
          (a.job.finishedOn ?? a.job.timestamp),
      )
      .slice(0, limit);

    return Promise.all(
      matches.map(async ({ job, state }) => {
        const info = await this.jobToInfo(job, 'list', state);
        return { ...info, queueName };
      }),
    );
   ...
   */
  async enqueueJob(request: TestJobRequest): Promise<TestJobResponse> {
    if (!this.registry) {
      throw new Error('No test registry configured');
    }

    if (request.type === 'job') {
      const job = this.registry.jobs.find(
        (entry) => jobRegistryId(entry) === request.id,
      );
      if (!job) {
        throw new Error(`Unknown job "${request.id}"`);
      }
    ...
    if (request.type === 'flow') {
      const flow = (this.registry.flows ?? []).find(
        (entry) => entry.id === request.id,
      );
      if (!flow) {
        throw new Error(`Unknown flow "${request.id}"`);
      }

      const input = flow.schema
        ? flow.schema.parse(request.data)
        : request.data;
    ...
    throw new Error(`Unknown test target type "${request.type}"`);
  }
  ...
  async runSchedulerNow(
    ...
    let rawOpts: StoredJobOpts = { ...scheduler.template?.opts };
    ...
    const job = await queue.add(scheduler.name, data ?? {}, opts);
  ...
  private extractTags(data: unknown): JobTags | undefined {
    ...
    for (const field of this.tagFields) {
      const value = getTagValue(data, field);
      if (value !== undefined) {
        tags[field] = value;
      }
    }
  ...
  async getTagValues(
    ...
    for (const jobs of queueResults) {
      for (const job of jobs) {
        const value = getTagValue(job.data, field);
  ...
  private async jobToInfo(
    ...
    let state: JobStatus | undefined = knownState;
    if (!state) {
    ...
    return this.cached(`flows:${limit}`, this.CACHE_TTL.flows, async () => {
        ...
        relevantQueues.map(async ({ queueName, queue }) => {
          try {
            ...
            const otherJobArrays = await Promise.all(
              otherTypes.map(async (type) => {
                try {
                  return await queue.getJobs(type, 0, 30); // Reduced from 100
                } catch {
                  return [];
                }
              }),
      ...
      for (
        ...
        const batchResults = await Promise.all(
          batch.map(async ({ queueName, job }): Promise<FlowSummary | null> => {
            try {
   ...
   */
  private async convertFlowTree(tree: JobNode): Promise<FlowNode> {
    const job = tree.job;
    const state = await job.getState();
    const duration =
      job.finishedOn && job.processedOn
   ...
   */
  private countFlowStats(tree: JobNode): {
    total: number;
    completed: number;
    failed: number;
  } {

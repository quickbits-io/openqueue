...
import type { JobStatus, TestJobRequest } from '@/core/types';
import { api } from './api';
...
export const queryKeys = {
  ...
  jobSpans: (queueName: string, jobId: string) =>
    ['job-spans', queueName, jobId] as const,
  runs: (
    sort?: string,
    filters?: {
      status?: JobStatus;
  ...
  schedulers: {
    ...
    dynamic: (sort?: string) => ['schedulers', 'dynamic', sort] as const,
    detail: (queueName: string, key: string) =>
      ['schedulers', 'detail', queueName, key] as const,
...
/**
 * Hook for fetching grouped error triage data
 */
export function useErrors() {
  return useQuery({
    queryKey: queryKeys.errors,
    queryFn: ({ signal }) => api.getErrors(signal),
...
/**
 * Hook for fetching persisted run spans (lazy — enable when Timeline tab is open)
 */
export function useJobSpans(
  queueName: string,
  jobId: string,
  options?: { enabled?: boolean; pollWhileActive?: boolean },
) {
  return useQuery({
    queryKey: queryKeys.jobSpans(queueName, jobId),
    queryFn: () => api.getJobSpans(queueName, jobId),
    enabled: options?.enabled ?? true,
    refetchInterval: options?.pollWhileActive ? 5000 : false,
  });
}
...
/**
 * Hook for fetching storage-backed dynamic schedules with sorting
 */
export function useDynamicSchedules(sort?: string, enabled = true) {
  return useQuery({
    queryKey: queryKeys.schedulers.dynamic(sort),
    queryFn: () => api.getDynamicSchedules(sort),
    enabled,
...
/**
 * Hook for fetching a single scheduler's detail (config, upcoming + recent runs)
 */
export function useSchedulerDetail(queueName?: string, key?: string) {
  return useQuery({
    queryKey: queryKeys.schedulers.detail(queueName ?? '', key ?? ''),
    queryFn: () => api.getSchedulerDetail(queueName!, key!),
    enabled: Boolean(queueName && key),
    refetchInterval: 5000,
  });
}
...
export function useRunDynamicSchedule() {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: (id: string) => api.runDynamicSchedule(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: queryKeys.schedulers.all });
      queryClient.invalidateQueries({ queryKey: queryKeys.runsAll });
      queryClient.invalidateQueries({ queryKey: queryKeys.queues });
...
export function useActivateDynamicSchedule() {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: (id: string) => api.activateDynamicSchedule(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: queryKeys.schedulers.all });
    },
  });
...
export function useDeactivateDynamicSchedule() {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: (id: string) => api.deactivateDynamicSchedule(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: queryKeys.schedulers.all });
    },
  });
...
export function useDeleteDynamicSchedule() {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: (id: string) => api.deleteDynamicSchedule(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: queryKeys.schedulers.all });
    },
  });
...
export function useTestJob() {
  ...
  return useMutation({
    mutationFn: (params: TestJobRequest) => api.testJob(params),
    onSuccess: (response) => {
      queryClient.invalidateQueries({
        queryKey: queryKeys.jobsAll(response.queueName),
      });
      queryClient.invalidateQueries({ queryKey: queryKeys.runsAll });
      queryClient.invalidateQueries({ queryKey: queryKeys.queues });

...
| `redis.url` | Redis connection string. Sugar for a BullMQ [world](/docs/worlds); **XOR** with `world`. |
| `redis.bullPrefix` | Override the BullMQ key prefix. |
| `world` | A [world](/docs/worlds) factory (e.g. `worldPostgres({ url })`) — the non-BullMQ path. XOR with `redis`. When set, the world owns durable state, so `storage` is not accepted. |
...
| `retention` | Age-based pruning of durable run history — `{ completed: 30, failed: 90, logs: 30 }` days by default, `false` keeps a category forever. See [Retention](/docs/persistence#retention). |
...
| `api.auth` | Ordered [`AuthStrategy`](/docs/upgrading-to-1.0#new-surfaces) walk for the control API. Empty array = always 401 (fail-closed). |
...
| `auth` | `{ username, password }` for basic auth, or an ordered [`AuthStrategy`](/docs/upgrading-to-1.0#new-surfaces) walk. Unset = dashboard open. |

...
The sweep is built for production backlogs: deletes run in bounded
batches (up to ~200k rows per category per sweep), so a first sweep over
millions of rows stays cheap and the hourly cadence drains the rest
incrementally. Replicas sharing a database coordinate through a Postgres
advisory lock — exactly one prunes per sweep — and the timers carry ±10%
jitter so a fleet doesn't fire in lockstep.
